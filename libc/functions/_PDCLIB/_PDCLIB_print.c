...
#define E_double   (1<<17)  // type is double
#define E_fixed    (1<<18)  // specifier was f(F) not g(G)
...
const char * _PDCLIB_print( const char * spec, struct _PDCLIB_status_t * status )
{
    ...
    */
    ...
    {
        int precision = status->prec == EOF ? 6 : status->prec;
        double val = status->flags & E_ldouble ? va_arg(status->arg, long double) : va_arg(status->arg, double);
        if(isnan(val))
        {
            status->prec = 10;
            stringformat(status->flags & E_lower ? "nan" : "NAN", status);
        }
        else if(isinf(val))
        {
            status->prec = 4;
            if(val < 0) stringformat(status->flags & E_lower ? "-inf" : "-INF", status);
            else stringformat(status->flags & E_lower ? "+inf" : "+INF", status);
        }
        else
        {
            // FIXME: long long is not enough
            long long v = (val * ipow(10, precision)) + (val < 0.0 ? -0.5 : 0.5);
            if(!(status->flags & E_fixed) && !v)
            {
                status->prec = 1;
                stringformat("0", status);
            }
            else
            {
                if(v < 0)
                ...
                for(int i = 19; i > 0; --i)
                {
                    ...
                    int dig = v / divider;
                    noskip |= dig != 0 || i == (precision + 1) || (!noskip && !(status->flags & E_fixed) && v < DBL_EPSILON);
                    v -= dig * divider;
                    divider /= 10;
                    if(!noskip) continue;
                    d[0] = '0' + dig;
                    status->prec = 1;
                    stringformat(d, status);
                    if(i <= (precision + 1) && !(status->flags & E_fixed) && v < DBL_EPSILON) break;
                }
            }
        }
    }
